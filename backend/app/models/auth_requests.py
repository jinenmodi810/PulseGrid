...
class RegisterUserRequest(BaseModel):
    ...
    home_zone_id: str = Field(default="", max_length=120)
    ...
    # Backward compatibility: older clients may still send zone_id / family_size only.
    zone_id: str | None = Field(default=None, max_length=120)
    family_size: int | None = Field(default=None, ge=0, le=50)

    @model_validator(mode="after")
    def _merge_legacy_aliases(self) -> RegisterUserRequest:
        hz = (self.home_zone_id or "").strip() or ((self.zone_id or "") or "").strip()
        if not hz:
            raise ValueError("home_zone_id is required")
        object.__setattr__(self, "home_zone_id", hz)
        if self.household_size is None and self.family_size is not None:
            object.__setattr__(self, "household_size", self.family_size)
        return self
...
class RegisterVictimAuthRequest(BaseModel):
    ...
    phone: str = Field(..., min_length=3, max_length=40)
    preferred_language: str = Field(default="en", max_length=32)
    home_zone_id: str = Field(..., min_length=1, max_length=120)
    household_size: int | None = Field(default=None, ge=1, le=50)
    elderly_count: int = Field(default=0, ge=0, le=50)
    mobility_concern: bool = False
    oxygen_dependency: bool = False
